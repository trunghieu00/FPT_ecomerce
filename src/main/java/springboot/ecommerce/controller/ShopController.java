...
//		model.addAttribute("listDiscount", discountService.getList(id));
	...
	public String getProductBannedPaging(Model model, @PathVariable(name = "page") int pageNum, HttpSession session,
		...
		if (keyword != null) {
			model.addAttribute("listProduct",
					productService.getProductByNameOrCodeBanned(keyword, pageNum, id).getContent());
			model.addAttribute("totalPage",
	...
	public String getProductPageBySearchBanned(Model model, @RequestParam(name = "keyword") String keyword,
			@RequestParam(name = "idShop") Integer id, HttpSession session) {
		session.setAttribute("listSearch",
				productService.getProductByNameOrCodeBanned(keyword, FIRST_PAGE, id).getContent());
		session.setAttribute("totalPage",
	...
	public String updateShopInfo(Model model, HttpSession session) {
		Integer id = (Integer) session.getAttribute("idShop");
		Optional<ShopEntity> shop = shopService.findShopById(id);
		model.addAttribute("shop", shop.get());
		session.setAttribute("nameShop", shop.get().getName());
		session.setAttribute("img1", shop.get().getPath1());
		session.setAttribute("img2", shop.get().getPath2());

		return "updateInfoShop";
	}

	@PostMapping(value = "/doUpdateShopInfo")
	public String doUpdateShop(Model model, @ModelAttribute(name = "shop") @Valid ShopEntity shop, HttpSession session,
			BindingResult result, HttpServletRequest request) {

		String i1 = (String) session.getAttribute("img1");
		String i2 = (String) session.getAttribute("img2");
		String path = request.getServletContext().getRealPath("");
		System.out.println(path);
		String savePath = path + saveDir;
		if (result.hasErrors()) {
		...
		try {
			MultipartFile multipartFile1 = shop.getMultipartFile1();
			MultipartFile multipartFile2 = shop.getMultipartFile2();
			...
			if (!fileName1.isEmpty()) {
				File file1 = new File(savePath, fileName1);
				multipartFile1.transferTo(file1);
				shop.setPath1("resources/images/" + fileName1);
			} else {
				shop.setPath1(i1);
			}
			...
			if (!fileName2.isEmpty()) {
				File file2 = new File(savePath, fileName2);
				multipartFile2.transferTo(file2);
				shop.setPath2("resources/images/" + fileName2);
			} else {
				shop.setPath2(i2);
			}
		...
		model.addAttribute("message", "Cập nhật thông tin shop thành công");
		return "redirect:/shop/updateShopInfo";
	}
